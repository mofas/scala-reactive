...
object WikipediaSuggest extends SimpleSwingApplication with ConcreteSwingApi with ConcreteWikipediaApi {
  ...
  def top = new MainFrame {
    ...
    // TO IMPLEMENT
    val suggestions: Observable[Try[List[String]]] = searchTerms.concatRecovered(v => wikiSuggestResponseStream(v))
    ...
    // TO IMPLEMENT
    val pages: Observable[Try[String]] = selections.concatRecovered  { x => wikiPageResponseStream(x) }

    // TO IMPLEMENT
    val pageSubscription: Subscription = pages.observeOn(eventScheduler) subscribe {
      x => x match {
        case Success(v) => editorpane.text_=(v)
